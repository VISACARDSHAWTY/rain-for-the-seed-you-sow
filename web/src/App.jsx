...
function ToggleSwitch({ label, checked, onChange, disabled = false }) {
  return (
    <label className="switchRow">
      <span>{label}</span>
      <input type="checkbox" className="switchInput" checked={checked} onChange={(e) => onChange(e.target.checked)} disabled={disabled} />
...
export default function App() {
  ...
  const [sensorPage, setSensorPage] = useState(0);   // pagination for sensor logs
  ...
  const fetchHistoryAndCharts = async () => {
    try {
      ...
      // Last 5 Days Bar Chart (fixed logic)
      const dayMap = {};
  ...
  useEffect(() => {
    if (activeTab === "history") {
      ...
      setSensorPage(0); // reset to first page when switching tab
    }
  }, [activeTab]);
  ...
  // Pagination for Sensor Logs (10 per page)
      ...
      <main className="page">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "20px", marginTop: "12px" }}>
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: "8px", margin: "12px 0" }}>
                  {[1,2,3,4].map(i => <div key={i} style={{width:"20px", height:"48px", border:"2.5px solid #67d18f", borderRadius:"6px", background: i <= tankBars ? "#67d18f" : "transparent"}} />)}
                </div>
        ...
        {activeTab === "history" && (
          <section className="card">
            ...
            {/* Sensor Logs History with Pagination */}
            ...
            {totalSensorPages > 1 && (
              <div style={{ display: "flex", justifyContent: "center", gap: "12px", marginTop: "15px" }}>
                ...
                <span style={{ alignSelf: "center", color: "#9de5be" }}>
                  Page {sensorPage + 1} of {totalSensorPages}
                </span>
                <button 
                  className="btn secondary" 
                  onClick={() => setSensorPage(p => Math.min(totalSensorPages - 1, p + 1))}
